...
TEST_F(GrayTest, test_two_b_two) {
	...
	const auto width_1 = 137U;
	const auto height_1 = 25U;

	test_kernel(width_1, height_1);

	const auto width_2 = 1025U;
	const auto height_2 = 32U;

	test_kernel(width_2, height_2);

	const auto width_3 = 48U;
	const auto height_3 = 65U;
...
TEST_F(GrayTest, test_two_c_one) {
	...
	const auto test_kernel = [construct_pixel](const auto width, const auto height) {
		...
		auto image = BitmapImage{ height, width };
		...
		try {
			for (auto y = 0; y < height; y++) {
				for (auto x = 0; x < width; x++) {
					const auto pixel = pixels_color[y * width + x];
		...
		const auto& computed_picture = get_grayscale_cuda(static_cast<const BitmapImage&>(image));
		...
		try {
			...
			for (auto y = 0; y < height; y++) {
				for (auto x = 0; x < width; x++) {
					const auto pixel_kernel = pixels_gray_gpu[y * width + x];
					const auto pixel_image = computed_picture.get_pixel(y, x);
					flag &= (pixel_kernel == pixel_image);
				}
			}
			...
			for (auto y = 0; y < height; y++) {
				for (auto x = 0; x < width; x++) {
					const auto pixel_kernel = pixels_gray_gpu[y * width + x];
					const auto pixel_image = computed_picture.get_pixel(x, y);
					flag &= (pixel_kernel == pixel_image);
				}
			}
